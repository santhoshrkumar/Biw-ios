...
class NotificationViewModelTest: XCTestCase {
    ...
    func testUnreadNotificationsCount() {
        let viewModel = NotificationViewModel(withRepository: NotificationRepository())
        guard let allSections = try? viewModel.sections.value() else { return }
        XCTAssertNotNil(viewModel.getUnreadNotificationsCount(allSections: allSections))
    }
   
    func testRemoveItem() {
        let viewModel = NotificationViewModel(withRepository: NotificationRepository())
        XCTAssertNotNil(viewModel.removeItem(at: IndexPath(item: 0, section: 0)))
        XCTAssertNotNil(viewModel.removeItem(at: IndexPath(item: 0, section: 0)))
        XCTAssertNotNil(viewModel.removeItem(at: IndexPath(item: 0, section: 0)))
        XCTAssertNotNil(viewModel.removeItem(at: IndexPath(item: 0, section: 0)))
        XCTAssertNotNil(viewModel.removeItem(at: IndexPath(item: 0, section: 0)))
    ...
    func testEditAction() {
        let viewModel = NotificationViewModel(withRepository: NotificationRepository())
        XCTAssertNotNil(viewModel.editAction(section: 0, buttonType: ButtonType.markAllAsRead))
        XCTAssertNotNil(viewModel.editAction(section: 0, buttonType: ButtonType.clearAll))
    }
    
    func testUpdateNotification() {
        let viewModel = NotificationViewModel(withRepository: NotificationRepository())
        //For unread notification it is not nil
        let detailUrlForUnRead = viewModel.updateNotification(indexPath: IndexPath(item: 0, section: 0))
        XCTAssertNotNil(detailUrlForUnRead)
        
        //For read notification it is nil
        let detailUrlForRead = viewModel.updateNotification(indexPath: IndexPath(item: 0, section: 1))
        XCTAssertNil(detailUrlForRead)
    }
}
