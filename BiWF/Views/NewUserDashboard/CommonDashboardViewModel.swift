...
class CommonDashboardViewModel {
    ...
    var currentAppointment: ServiceAppointment? {
        didSet {
            // In service appointment, if the state = cancelled then show new user dashboard
            if currentAppointment?.getState() == .cancelled && currentAppointment?.getAppointmentType() == .service {
                moveToStandardDashboardSubject.onNext(())
            } else {
                setSections()
            }
        }
